...
const SearchForm = ({searchTerm,setSearchTerm,searchHandler,isSearching})=>{
    return (
            <div className="flex md:flex-col mt-0 space-y-3 sm:space-y-0 sm:flex-row sm:justify-center sm:-mx-2">
                ...
                <form className="relative max-w-2xl mx-auto" onSubmit={searchHandler}>
                        ...
                        <div className="relative"> <span className="material-symbols-outlined absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 text-2xl"> search </span> 
                            ...
                            <button type="submit" disabled={isSearching} className="absolute right-3 top-1/2 transform -translate-y-1/2 bg-gradient-to-r from-pink-500 to-violet-500 hover:from-pink-600 hover:to-violet-600 p-3 rounded-lg transition-all duration-300 hover:scale-105 disabled:cursor-wait disabled:opacity-70">
                            {isSearching ? <span className="text-white text-sm">Searching</span> : <span className="material-symbols-outlined text-white">arrow_forward</span>} </button></div>
                        </div>
                </form>
            </div>
        )
